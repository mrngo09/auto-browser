export function generateSecChUa(): string {
    ...
    const secChUaParts = selectedBrands.map(brand => {
        const version = brand === 'Not(A:Brand' ? Math.floor(Math.random() * 50) + 1 : getRandomVersion(); // Lower versions for fallback
...
export function generateUserAgent(): string {
    ...
    const device = devices[Math.floor(Math.random() * devices.length)] ? ` (${devices[Math.floor(Math.random() * devices.length)]})` : '';
